...
class PID:
    ...
    def __init__(self, target, frames, position, rotation, eulerAngles, gain, Size = [1,1,2*numpy.pi,1],SamplingFreq = 250):
        self.Size = Size
        self.SamplingFreq = SamplingFreq
        dt,Position,PositionRatedt,Rotation,RotationRatedt,Euler,EulerRatedt = FilteredAverage.FilteredAverage(frames, position, rotation, eulerAngles)
...
#            print(valuesrc[2])
            ControlInput = [valuesrc[1]-0.55,valuesrc[2]-0.55,valuesrc[3]-0.55,valuesrc[5]-0.1755]
...
#        print(Euler[1])
        X_HeadingRate = -numpy.cos(Euler[1])*PositionRate[0]+numpy.sin(Euler[1])*PositionRate[1]
